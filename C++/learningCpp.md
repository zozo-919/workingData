...
### 面向对象程序设计
  ...
  - **区别：**
    ...
    - **面向对象： 程序的基本元素是对象**
        ...
        - **面向对象程序设计的基本特征**：
            1. **抽象**： 程序的对象是抽象的，不依赖于具体的实现细节，只关注对象所应具备的功能和属性
            2. **封装**： 把数据和显示操作的代码集中起来放在对象内部 尽可能隐藏对象的内部实现细节 信息隐藏
            3. **继承**： 允许创建新的类从已有的类继承其属性和方法 使得类之间的关系更加紧密
            4. **多态**： 允许不同类的对象对同一消息作出不同的响应 使得程序具有更好的灵活性

###  编译器编译过程
- 编译
    1. 预编译: 注释删除、宏替换、头文件包含、条件编译等
    2. 编译:   语法检查、符号汇总
    3. 汇编:   符号表生成 (存有符号的名称和其存储在内存中的地址) (若某个符号仅声明未定义则符号表中就会存储一份虚拟地址)
- 链接
    1. 合并段表
    2. 符号表的合并及重定位
...
- `cin`
    - ==**标准输入流**==  用于从控制台读取输入数据
    - **输入运算符 ==>==**
        - 默认情况下跳过空白符 读入后面与变量类型对应的值(给一组变量赋值时可用空格符 回车符 制表符将输入数据间隔开)
...
- `::`
    ...
    - **假设声明了一个类`Zozo` 声明了一个成员函数 `zozoFunc()` 在类中未给出定义 则类外定义时使用`类名::成员函数名(形参表)`**
        - **使用示例**
        ```cpp
            class Zozo {
            public:
...
- `const` ==**常量限定符**==
  - 修饰常量  const对象必须初始化
    ...
    - **常指针**
      - 将指针变量所指的地址声明为常量  `const指针 非const数据`
        - **使用示例**
        ```cpp
...
- **总结: ==左定值 右定向 const修饰不变量==  常量一旦建立 任何地方都不能修改**
...
- `new` 和 `delete`
  - 同c语言中的 `malloc` 和 `free` 函数  分配和释放堆区内存
    ...
    - **引用**
      ...
      - **注释**
        - 引用非独立数据类型 必须与某一变量联系 且声明时需立即初始化
...
- ==**类与对象**==
  - **类的构成**
    ...
    - **注意**
      - 一般的 一个类的数据成员声明为私有成员 成员函数声明为公有成员
...
- ==**内联成员函数定义**==
  ...
  - ***显示声明*** 在类声明中只给函数原型 定义在类外部
    ...
    - **注意**  
        用inline定义的内联函数 声明和定义须在同一文件中 否则编译器无法区分
...
- **对象的定义及使用**
  ...
  - **对象中成员的访问** 
    ...
    - **注意**
      ...
      - 定义对象时 若是指向此对象的**指针变量** 则用 ==->== 操作符访问成员对象
        ```cpp
            Score *p ;
            p->setScore(80, 90);
        ```
...
- **string 类** 
  ...
  - `size` ==**获取字符串长度**==
    ...
    ```
    - **注释**: 一条表达式中若有了 `size()` 函数 则不需要使用 `int` 避免混用`int`和`unsigned`
...
- **继承与派生**
  ...
  - 派生类对基类成员的访问:
    ...
    - 派生类可声明与基类成员同名的成员 在无虚函数时 若派生类中定义了与基类成员同名的成员 则派生类成员覆盖基类的同名成员 在派生类中使用该名字代表访问派生类中声明的成员
    - 若想在派生类中使用与基类同名的成员 须在成员名之前加上 基类名及作用域标识符 `::`  即  ` 基类名::函数名(参数表);`
        ```cpp
            class B : private A{
                private:
                    int Bb;
...
- **函数模板与类模板**     ` 将类型定义为参数 实现代码重用机制的工具 `
  - **函数模板**
    ...
    - `template` -- 声明模板的关键字  '类型参数' -- 常用T 、type等 使用函数模板时 必须将类型实例化
      ...
      - 在函数模板中允许使用多个类型参数 template 定义部分的每个类型参数前必须有关键字 `typename或class`
...
- **C++的输入和输出**   `以字节流形式实现`
  - **优点** 
    - 完全支持c语言的输入输出系统 建立面向对象的输入输出系统 比c更安全可靠
  ...
  - **文件的输入/输出**   `'文件' --存放在外部介质上的数据的集合`
    ...
    - **检测文件结束**
      ...
      - 键盘上输入字符时 其结束符是 `Ctrl+Z`  按下 `Ctrl+Z` 组合键  `eof()`函数返回的值为`真`
...
- **STL标准模板库** 
  - STL(Standard Template Library) -- **标准模板库** 是一个模板集合 包含了常用的数据结构和算法
    ...
    - `push_back()函数` 向vector中添加元素
        ...
        ```
      - 注意: vector下标可用于访问已存在的元素 但不能用于添加元素 当vector是空时 只能用push_back()添加或访问元素

    - `lis` 容器
      - 头文件: `#include <list>`
      - 格式：  `list<类型> 变量名;`
        ```cpp
            list<int> lst;   // 声明一个元素类型为int的list
        ```
    ...
    - `stack`
      ...
      - 格. 式: `stack<类型> 变量名;`
        ```cpp
            stack<int> stk;   // 声明一个元素类型为int的stack
        ```
    - `queue`
      - 头文件: `#include <queue>`
      - 格  式: `queue<类型> 变量名;`
        ```cpp
            queue<int> que;   // 声明一个元素类型为int的queue
        ```

    - `priority_queue` **优先队列**
      - 头文件: `#include <queue>`
      - 格  式: `priority_queue<类型> 变量名;`
        ```cpp
            priority_queue<int> pq;   // 声明一个元素类型为int的优先队列
        ```
    ...
    - `map`
      ...
      - 格  式: `map<类型1,类型2> 变量名;`
        ```cpp
            map<string,int> mp;   // 声明一个元素类型为string,值类型为int的map
        ```
