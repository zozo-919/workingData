...
uint32_t lcd_read_point(uint16_t x, uint16_t y)
{
    ...
    if(lcddev.id == 0X9341 || lcddev.id == 0X5310 || lcddev.id == 0X1963 || lcddev.id == 0x7789)
    {
      lcd_wr_regno(0X2E);       /* 9341/3510/1963/7789 发送读GRAM指令 */
    }
...
void lcd_display_on(void)
{
    ...
    else if(lcddev.id == 0X9341 || lcddev.id == 0X5310 || lcddev.id == 0X1963 || lcddev.id == 0x7789)
    {
...
void LCD_Fast_DrawPoint(uint16_t x, uint16_t y, uint32_t color)
{
    ...
    else if (lcddev.id == 0X9341 || lcddev.id == 0X5310 || lcddev.id == 0x7789)
    {
        lcd_wr_regno(lcddev.setxcmd); 
        lcd_wr_data(x >> 8);
        lcd_wr_data(x & 0XFF);
    ...
    {
        lcd_wr_data(lcddev.setxcmd);
        lcd_wr_data(x >> 8);  
        lcd_wr_data(lcddev.setxcmd + 1);
        lcd_wr_data(x & 0XFF);  
        lcd_wr_data(lcddev.setycmd);
        lcd_wr_data(y >>  8);  
        lcd_wr_data(lcddev.setycmd + 1);
        lcd_wr_data(y & 0XFF); 
    }
...
void lcd_init(void)
{
    ...
    else
    {
        ...
        /* FMC读时序控制寄存器 */
        fmc_read_handle.AddressSetupTime = 0x0F;          /* 地址建立时间(ADDSET)为15个HCLK 1/180M*15=5.5ns*15=82.5ns */
        fmc_read_handle.AddressHoldTime = 0x00;
        fmc_read_handle.DataSetupTime = 0x46;             /* 数据保存时间(DATAST)为70个HCLK=5.5ns*70=385ns */
                                                          /* 因为液晶驱动IC的读数据的时候,速度不能太快,尤其是个别奇葩芯片 */
        fmc_read_handle.AccessMode = FMC_ACCESS_MODE_A;   /* 模式A */
        /* FMC写时序控制寄存器 */
        fmc_write_handle.AddressSetupTime = 0x0F;         /* 地址建立时间(ADDSET)为15个HCLK=82.5ns */
        fmc_write_handle.AddressHoldTime = 0x00;
        fmc_write_handle.DataSetupTime = 0x0F;            /* 数据保存时间(DATAST)为15个HCLK=82.5ns */
